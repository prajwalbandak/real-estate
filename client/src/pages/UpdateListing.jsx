...
const handleSubmit = async(e) =>{
  ...
  try{
    ...
    const result = await fetch(`/api/updateListing/${params.listingId}`, {
      method:'POST',
      headers: {
...
useEffect(() =>{
    const fetchListing = async() =>{
        ...
        const response = await fetch(`/api/getListingForUser/${listingId}`);
        const data = await response.json();
        if(data.success === false){ 
            console.log("the error while getting the getlisting for user" + data.message);
            return;
        }
        setFormData(data);
    }
  ...
  return (
    <main className='max-w-4xl p-3 mx-auto'>
      <h1 className='text-3xl text-center bg-transparent font-semibold my-6'> Update Listing</h1>
